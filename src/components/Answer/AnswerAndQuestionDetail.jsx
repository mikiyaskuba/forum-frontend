...
const AnswerAndQuestionDetail = () => {
		...
		const fetchQuestionAndAnswers = async () => {
			try {
				const questionResponse = await axios.get(
					`https://forum-backend-1-c5of.onrender.com/api/questions/questions/${id}`
				);
				setQuestion(questionResponse.data.question);

				const answersResponse = await axios.get(
					`https://forum-backend-1-c5of.onrender.com/api/answers/questions/${id}/answers`
				);
	...
	const handleAnswerDelete = async (answerid) => {
		...
		try {
			await axios.delete(
				`https://forum-backend-1-c5of.onrender.com/api/answers/answers/${answerid}`,
				{
					headers: {
						Authorization: `Bearer ${token}`,
					},
